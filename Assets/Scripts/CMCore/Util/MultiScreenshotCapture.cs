...
namespace CMCore.Util
{
				...
				set
				{
					if( m_isActive != value )
					{
						...
						if( m_isActive )
						{
							...
							newIndex =
 (int) SizeHolder.CallMethod( "IndexOf", customSize ) + (int) SizeHolder.CallMethod( "GetBuiltinCount" );
			...
			public CustomResolution( int width, int height , string name)
			{
				this.width = width;
				this.height = height;
				this.name = name;
		...
		private class Resolution
		{
			...
			public Resolution(Vector2 resolution, string name)
			{
		...
		private List<Resolution> resolutions =
			new List<Resolution>()
			{
				new Resolution(new Vector2( 1024,768), "default")
				
			}; // Not readonly to support serialization
		...
		private void OnGUI()
		{
			...
			{
				resolutions.Insert( 0, new Resolution(new Vector2(), "new") );
			...
			for( int i = 0; i < resolutions.Count; i++ )
			{
				...
				resolutions[i].resolution = EditorGUILayout.Vector2Field( GUIContent.none, resolutions[i].resolution );
				resolutions[i].name = EditorGUILayout.TextField("N: ", resolutions[i].name);
				GUI.enabled = true;
				resolutionsEnabled[i] = EditorGUILayout.Toggle( GUIContent.none, resolutionsEnabled[i], GL_WIDTH_25 );

				if( GUILayout.Button( "+", GL_WIDTH_25 ) )
				{
					resolutions.Insert( i + 1, new Resolution(new Vector2(), "new") );
 ...
 EditorGUILayout.ToggleLeft( "Allow transparent background", allowTransparentBackground );
		...
		private void CaptureScreenshot( Resolution resolutions )
		{
			int width = Mathf.RoundToInt( resolutions.resolution.x * resolutionMultiplier );
			int height = Mathf.RoundToInt( resolutions.resolution.y * resolutionMultiplier );

			if( width <= 0 || height <= 0 )
				Debug.LogWarning( "Skipped resolution: " + resolutions.resolution );
			else if( !captureOverlayUI || targetCamera == TargetCamera.SceneView )
				CaptureScreenshotWithoutUI( width, height , resolutions.name);
			else
				queuedScreenshots.Add( new CustomResolution( width, height , resolutions.name) );
		}
		...
		private void CaptureQueuedScreenshots()
		{
			...
			else
			{
				try
				{
					CaptureScreenshotWithUI(resolution.name);
				}
				catch( Exception e )
				{
		...
		private void CaptureScreenshotWithoutUI( int width, int height ,string name )
		{
			...
			try
			{
				...
				screenshot =
 new Texture2D( renderTex.width, renderTex.height, saveAsPNG && allowTransparentBackground ? TextureFormat.RGBA32 : TextureFormat.RGB24, false );
				screenshot.ReadPixels( new Rect( 0, 0, renderTex.width, renderTex.height ), 0, 0, false );
				screenshot.Apply( false, false );

				File.WriteAllBytes( GetUniqueFilePath( renderTex.width, renderTex.height, name ), saveAsPNG ? screenshot.EncodeToPNG() : screenshot.EncodeToJPG( 100 ) );
			}
			finally
			{
		...
		private void CaptureScreenshotWithUI(string name)
		{
			RenderTexture temp = RenderTexture.active;
			...
			try
			{
				...
				screenshot =
 new Texture2D( width, height, saveAsPNG && allowTransparentBackground ? TextureFormat.RGBA32 : TextureFormat.RGB24, false );
				...
				File.WriteAllBytes( GetUniqueFilePath( width, height , name), saveAsPNG ? screenshot.EncodeToPNG() : screenshot.EncodeToJPG( 100 ) );
			}
			finally
			{
				RenderTexture.active = temp;
		...
		private void LoadSettings()
		{
			...
			string oneLevelUp = Path.Combine (System.IO.Directory.GetCurrentDirectory(),"resolutions.json");
	
			
			if( !string.IsNullOrEmpty( oneLevelUp ) )
			{
				if (!File.Exists(oneLevelUp)) return;
				SaveData saveData = JsonUtility.FromJson<SaveData>( File.ReadAllText( oneLevelUp ) );
				resolutions = saveData.resolutions ?? new List<Resolution>();
				resolutionsEnabled = saveData.resolutionsEnabled ?? new List<bool>();
			}
		}

		private string GetUniqueFilePath( int width, int height, string name )
		{
			
			string filename = string.Concat(name , " {0}", saveAsPNG ? ".png" : ".jpeg")  ;
